  ...
  return (
    ...
    >
      ...
      <div className="bg-slate-900/40 border border-slate-800/60 backdrop-blur-md rounded-2xl p-4 md:p-6 shadow-sm text-right">
        ...
        <p className="text-xs text-slate-400 font-medium mt-1">كشف عام مجمع بمدفوعات ومبيعات وحسابات ذمم العملاء للشهر الحالي</p>
      </div>
        ...
        <table className="w-full border-collapse text-xs text-center">
          ...
          <tfoot>
            <tr className="bg-slate-950/90 font-black text-slate-100 border-t-2 border-amber-500 select-none">
              <td className="py-4 px-4 text-amber-400 text-xs text-right">إجمالي الشهر</td>
              <td className="py-4 px-3 font-mono text-sm">{Math.round(grand.tw)} كغ</td>
              <td className="py-4 px-3 font-mono text-sm">{Math.round(grand.nw)} كغ</td>
              <td className="py-4 px-3 font-mono text-sm text-amber-400">{fmt(grand.amt) || '—'}</td>
              <td className="py-4 px-3 font-mono text-sm text-emerald-400">{fmt(grand.paid) || '—'}</td>
              <td className={`py-4 px-3 font-mono text-sm ${grand.amt - grand.paid > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
