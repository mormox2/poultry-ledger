...
import { MONTHS, getRows, getTotals, calcBalance, fmt, getPreviousMonthsBalance } from '../js/utils';

export default function SupplierStatementPrint({ state, supplierId, onClose }) {
  ...
  const aggregatedData = useMemo(() => {
    let prevBalance = getPreviousMonthsBalance(state.purchases || {}, sup.id, startYear, startMonth);
    ...
    monthsRange.forEach(({ year, month }) => {
      const totals = getTotals(state.purchases || {}, sup.id, year, month);
      const rows = getRows(state.purchases || {}, sup.id, year, month);

      totalPurchases += totals.amt;
    ...
    const netPeriodBalance = totalPurchases - totalPaid;
  ...
  }, [monthsRange, sup.id, state.purchases, startYear, startMonth]);
  ...
  const statementSerial = `RELEVE-FOURN-${startYear}${String(startMonth).padStart(2, '0')}-${endYear}${String(endMonth).padStart(2, '0')}-${String(sup.id).slice(-4).padStart(4, '0')}`;
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`releve_fournisseur_${sup.name.replace(/\s+/g, '_')}_${startYear}_${startMonth}_to_${endYear}_${endMonth}.pdf`);
    } catch (err) {
      console.error("Failed to generate Supplier Statement PDF:", err);
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <h2 style={{ fontSize: '15px', fontWeight: '900', color: '#f59e0b', margin: 0 }}>
            📑 كشف حساب مورد مفصل (Relevé de Compte Fournisseur)
          </h2>
          <button 
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          <h3 style={{ fontSize: '13px', color: '#b45309', margin: '0 0 10px 0', fontWeight: '800', borderBottom: '1px dashed #cbd5e1', paddingBottom: '4px' }}>
            👤 مراجع الحساب وبيانات المورد:
          </h3>
          <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse' }}>
            <tbody>
              <tr>
                <td style={{ textAlign: 'right', padding: '4px 0', fontWeight: '600', width: '110px', color: '#475569' }}>اسم المورد (Fournisseur):</td>
                <td style={{ textAlign: 'right', padding: '4px 0', fontWeight: '700', color: '#1e293b', fontSize: '13px' }}>{sup.name}</td>
                <td style={{ textAlign: 'right', padding: '4px 0', fontWeight: '600', width: '115px', color: '#475569' }}>رقم الهاتف (Tel):</td>
                <td style={{ textAlign: 'right', padding: '4px 0', color: '#1e293b', fontWeight: '650' }}><span dir="ltr">{sup.phone}</span></td>
              </tr>
              <tr>
                <td style={{ textAlign: 'right', padding: '4px 0', fontWeight: '600', color: '#475569' }}>العنوان (Adresse):</td>
                <td style={{ textAlign: 'right', padding: '4px 0', color: '#1e293b' }}>{sup.address}</td>
                <td style={{ textAlign: 'right', padding: '4px 0', fontWeight: '600', color: '#475569' }}>المعرف الجبائي (M.F.):</td>
                <td style={{ textAlign: 'right', padding: '4px 0', color: '#1e293b', fontWeight: '650' }}><span dir="ltr">{sup.taxId || '—'}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div style={{
          ...
          {[
            { title: "رصيد سابق مستحق", val: `${fmt(aggregatedData.prevBalance)} د.ت`, sub: "مستحقات ما قبل الفترة", border: "border-slate-200 text-slate-800" },
            { title: "مشتريات الفترة الجارية", val: `${fmt(aggregatedData.totalPurchases)} د.ت`, sub: "قيمة البضائع المتسلمة", border: "border-amber-200 text-amber-600" },
            { title: "المدفوعات والمسدد له", val: `${fmt(aggregatedData.totalPaid)}.ت`, sub: "إجمالي المقبوضات النقدا", border: "border-emerald-200 text-emerald-600" },
            { title: "الرصيد المتبقي للمورد", val: `${fmt(aggregatedData.finalBalance)} د.ت`, sub: "الباقي المعلق له بذمتنا", border: "border-rose-200 text-rose-600 bg-rose-50/20" }
          ].map((st, i) => (
        ...
        <h3 style={{ fontSize: '12px', fontWeight: '900', color: '#b45309', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
          📊 خلاصة الحساب المشتريات الشهرية (Monthly Purchases Summary)
        </h3>
        ...
        <h3 style={{ fontSize: '12px', fontWeight: '900', color: '#b45309', marginBottom: '10px' }}>
          📋 كشف شراء تفصيلي بالأيام النشطة (Detailed Purchases Statement)
        </h3>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
          <thead>
            <tr style={{ background: '#f1f5f9', borderBottom: '2px solid #d4a843' }}>
              ...
              <th style={{ padding: '6px', textAlign: 'center', fontWeight: '850', color: '#10b981' }}>المدفوع له</th>
        ...
        <div style={{ 
          ...
          <div style={{ textAlign: 'left', maxWidth: '300px' }}>
            ...
            <p style={{ margin: '4px 0 0 0', fontSize: '9.5px', color: '#64748b', lineHeight: '1.4' }}>
              {state.companyInfo?.invoiceFooter || "تم إصدار كشف الحساب المشتريات المالي آلياً بواسطة تطبيق Dawajin Pro المشفر. المبالغ الموضحة نهائية وتعتبر معتمدة قانونياً."}
            </p>
          </div>
        </div>
